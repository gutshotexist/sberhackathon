...
async function main() {
  ...
  let   aggregator = new ethers.Contract(process.env.AGGREGATOR, abi, signer);

  aggregator.on("Round", async (roundId) => {
    ...
    aggregator.transmit(roundId, now, price, sig.v, sig.r, sig.s)
    .then((value)=>{
